...
async def cmd_start_or_restart(msg: Message, state: FSMContext):
    ...
    #     return
    user = get_user(db, msg.from_user.id)
    if msg.from_user.id == config.DEFAULT_ADMIN_USER_ID:
        if user is None:
            await msg.answer(YOU_ARE_ADMIN, reply_markup=admin_start_keyboard1())
        else:
            await msg.answer(YOU_ARE_ADMIN, reply_markup=admin_start_keyboard2())
        return
...
async def add_lesson_prompt_start(msg: Message, state: FSMContext):
    await msg.answer(ENTER_LESSON_START_PROMPT)
...
async def process_lesson_start_input(msg: Message, state: FSMContext):
    ...
    await msg.answer(ENTER_LESSON_END_PROMPT)
...
async def save_new_lesson(msg: Message, state: FSMContext):
    ...
    await save_schedule_and_exit(db, msg, state)
...
async def remove_lesson_prompt(msg: Message, state: FSMContext):
    ...
    for i, lesson in enumerate(sorted(editable, key=lambda a: time.fromisoformat(a["start"])), 1):
        start = datetime.combine(msg.date.date(), time.fromisoformat(lesson["start"]), tzinfo=pytz.UTC).astimezone(pytz.timezone("Europe/Moscow"))
        end = datetime.combine(msg.date.date(), time.fromisoformat(lesson["end"]), tzinfo=pytz.UTC).astimezone(pytz.timezone("Europe/Moscow"))
        lines.append(f"{i}. {start.strftime("%H:%M")} – {end.strftime("%H:%M")}")
    await msg.answer(SCHEDULE_CURRENT.format("\n".join(lines)))
    await msg.answer(ENTER_LESSON_NUMBER_TO_REMOVE)
...
async def process_remove_lesson(msg: Message, state: FSMContext):
    ...
    start = datetime.combine(msg.date.date(), time.fromisoformat(removed["start"]), tzinfo=pytz.UTC).astimezone(
        ...
        pytz.timezone("Europe/Moscow"))
    await msg.answer(LESSON_REMOVED.format(idx, start, end))
    await state.set_state(Form.schedule_editing)
    await msg.answer(SCHEDULE_EDITOR_CHOOSE_ACTION, reply_markup=schedule_edit_keyboard())
    await save_schedule_and_exit(db, msg, state)
...
async def show_editable_schedule(msg: Message, state: FSMContext):
    ...
    if not editable:
        ...
        return
    lines = []
    for i, lesson in enumerate(editable, 1):
        start = datetime.combine(msg.date.date(), time.fromisoformat(lesson["start"]), tzinfo=pytz.UTC).astimezone(pytz.timezone("Europe/Moscow"))
        end = datetime.combine(msg.date.date(), time.fromisoformat(lesson["end"]), tzinfo=pytz.UTC).astimezone(pytz.timezone("Europe/Moscow"))
        lines.append(f"{i}. {start.strftime('%H:%M')} – {end.strftime('%H:%M')}")
...
async def mark_current_lesson(msg: Message, state: FSMContext):
    ...
    if marker is not None:
        await msg.answer(
            ALREADY_MARKED_FOR_CURRENT_LESSON.format(tm[0] + 1, tm[1].astimezone(pytz.timezone("Europe/Moscow")).strftime(
                                                         "%H:%M"),
...
async def get_marks_current_lesson(msg: Message, state: FSMContext):
    ...
    line = MARKED_NOW_HEADER.format(tm[0] + 1, tm[1].astimezone(pytz.timezone("Europe/Moscow")).strftime('%H:%M'), tm[2].astimezone(pytz.timezone("Europe/Moscow")).strftime('%H:%M'))
...
async def get_marks_today(msg: Message, state: FSMContext):
    ...
    for s in schedule:
        ...
        line += MARKED_LESSON_LINE.format(schedule.index(s) + 1, dts.astimezone(pytz.timezone("Europe/Moscow")).strftime('%H:%M'), dte.astimezone(pytz.timezone("Europe/Moscow")).strftime('%H:%M'))
        # if not markers_in_lesson:
        #     line += "Никто не отметился"
        for m in markers_in_lesson:
            user = get_user(db, m.user_id)
            line += MARKED_USER_LINE.format(user.user_fio, m.add_timestamp.astimezone(pytz.timezone("Europe/Moscow")).strftime('%H:%M'))
        line += SEPARATOR
    await msg.answer(line, reply_markup=keyboard)
